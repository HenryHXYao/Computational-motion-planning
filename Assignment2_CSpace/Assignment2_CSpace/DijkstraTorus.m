...
while true
    ...
    for k = i-1:2:i+1
        ...
        if map(kk,j) ==2 || map(kk,j) ==3 || map(kk,j) ==5
            continue
        else
            if distances(kk,j) > min_dist + 1                
                distances(kk,j) = min_dist + 1;
                parent(kk,j) = current;
                map(kk,j) = 4;    
            end                                  
        end                                
    ...
    for k = j-1:2:j+1
        ...
        if  map(i,kk) ==2 || map(i,kk) ==3 || map(i,kk) ==5
            continue
        else
            if distances(i,kk) > min_dist + 1                
                distances(i,kk) = min_dist + 1;
                parent(i,kk) = current;
                map(i,kk) = 4;      
            end                                  
        end                                
